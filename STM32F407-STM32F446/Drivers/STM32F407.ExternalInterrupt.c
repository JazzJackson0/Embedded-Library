...
void EXTIInit(uint8_t extiNum, char port, E_Type interruptType, E_TriggerEdge triggerSelect) {
    ...
    uint8_t vecTablePos[] = {6, 7, 8, 9, 10, 23, 23, 23, 23, 23, 40, 40, 40, 40, 40, 40};
    TurnOnInterrupts();
    ...
    if (interruptType == SOFTWARE_INTERRUPT) {
        ...
        triggerState(extiNum, write); // Clear Pending Flag to ignore any previous events
    }
    ...
    // Hardware Interrupt
        ...
        // TO DO: Configure the enable and mask bits that control the NVIC IRQ channel mapped to the 
        ...
        NVICInterruptInit(vecTablePos[extiNum], 0, HARDWARE); // NVIC Info: See 1-Datasheets/STM32/'STM32 M4 Programming Manual.pdf'
