...
static Matrix Get_ExpectedStateEstimate(KalmanFilter k_filter) {
    ...
    return AddMatrices(MultiplyMatrices(k_filter.state_trans_matrix, k_filter.updatedStateEstimate), 
        MultiplyMatrices(k_filter.input_matrix, k_filter.input_vector));
}

static Matrix Get_ExpectedCovariace(KalmanFilter k_filter) {

    return AddMatrices(MultiplyMatrices(MultiplyMatrices(k_filter.state_trans_matrix, 
...
static Matrix Get_Innovation(KalmanFilter k_filter, Matrix sensorValues) {
    ...
    return SubtractMatrices(sensorValues, 
        MultiplyMatrices(k_filter.measurement_matrix, k_filter.expectedStateEstimate)); 
}
...
static Matrix Get_InnovationCovariance(KalmanFilter k_filter) {
    ...
    return AddMatrices( 
        MultiplyMatrices(
            MultiplyMatrices( k_filter.measurement_matrix, k_filter.expectedCovariance), 
...
static Matrix Get_KalmanGainMatrix(KalmanFilter k_filter) {
    ...
    Matrix result = MultiplyMatrices(
        MultiplyMatrices(k_filter.expectedCovariance, 
            Get_MatrixTranspose(k_filter.measurement_matrix)), inverse);
...
static Matrix Get_UpdatedCovariance(KalmanFilter k_filter) {
    ...
    return SubtractMatrices(
        k_filter.expectedCovariance, MultiplyMatrices(
            MultiplyMatrices(k_filter.kalmanGainMatrix, k_filter.measurement_matrix),
                k_filter.expectedCovariance));
}

static Matrix Get_UpdatedStateEstimate(KalmanFilter k_filter) {

    return AddMatrices(k_filter.expectedStateEstimate, 
        MultiplyMatrices(k_filter.kalmanGainMatrix, k_filter.innovation));
}
